...
//#include "../include/parameters.hpp"    //COMMIT CHANGE HERE
...
int main(int argc, char **argv)
{
	...
	while (rclcpp::ok())
	{
		...
		//cerr<<"Initial conditions : "<<endTime - startTime<<endl;
		std::string endTime_startTime = std::to_string(endTime - startTime); 
		std::string Initial_Conditions = "Initial Conditions: ";
		std::string Initial_Conditions_str = Initial_Conditions + endTime_startTime;
		RCLCPP_INFO(node->get_logger(), Initial_Conditions_str);
		...
		//cerr<<"frenet optimal planning: "<<endTime2-startTime2<<endl;
		std::string endTime2_startTime2 = std::to_string(endTime2-startTime2);
		std::string frenet_optimal_planning = "frenet optimal planning: ";
		std::string frenet_optimal_planning_str = frenet_optimal_planning + endTime2_startTime2;
		RCLCPP_INFO(node->get_logger(), frenet_optimal_planning_str);
		...
		double time_plot = omp_get_wtime();
		if (true)
		{
			plt::ion();
			plt::show();
		...
		double time_plot2 = omp_get_wtime();
		//cerr<<"Plotting graph : "<<time_plot2-time_plot<<endl;
		std::string time_plot2_time_plot = std::to_string(time_plot2-time_plot);
		std::string Plotting_graph = "Plotting graph: ";
		std::string Plotting_graph_str = Plotting_graph + time_plot2_time_plot;
		RCLCPP_INFO(node->get_logger(), Plotting_graph_str);
		...
		//cerr<<"Publishing to topics: "<<time_2-time_1<<endl;
		std::string time_2_time_1 = std::to_string(time_2 - time_1); 
		std::string Publishing_to_topics = "Publishing to topics: ";
		std::string Publishing_to_topics_str = Publishing_to_topics + time_2_time_1;
		RCLCPP_INFO(node->get_logger(), Publishing_to_topics_str);
		ctr++;
		 if(!run_frenet){
		...
		//cerr<<"Main loop: "<<mainLoop2-mainLoop1<<endl;
		std::string mainLoop2_mainLoop1 = std::to_string(mainLoop2-mainLoop1);
		std::string Main_loop = "Main loop: ";
		std::string Main_loop_str = Main_loop + mainLoop2_mainLoop1;
		RCLCPP_INFO(node->get_logger(), Main_loop_str);
	}
	return 0;
}
